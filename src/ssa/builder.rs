use super::{module::Module, basic_block::{BasicBlockId, BasicBlock, Terminator}, instruction::{Value, Instruction, BinaryOp}};
...
impl ModuleBuilder {
    ...
    pub fn push_bb(&mut self) -> BasicBlockId {
        self.module.blocks.push(BasicBlock::new(Vec::new(), Vec::new(), Terminator::NoTerminator));
    ...
    pub fn load_int(&mut self, var: Value, int: i32) {
        if let Some(id) = self.current_block {
            self.module.blocks[id.0].instructions.push(Instruction::Int(var, int));
        } else {
    ...
    pub fn build_move(&mut self, from: Value, to: Value) {
        if let Some(id) = self.current_block {
            self.module.blocks[id.0].instructions.push(Instruction::Move(to, from));
        } else {
    ...
    pub fn build_binop(&mut self, to: Value, lhs: Value, rhs: Value, operation: BinaryOp) {
        if let Some(id) = self.current_block {
            self.module.blocks[id.0].instructions.push(Instruction::BinOp(operation, to, lhs, rhs));
        } else {
            panic!("Malformed IR: Tried to build a move without a basic block selected.");
        }
    }
